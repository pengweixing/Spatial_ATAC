...
echo -e "rm $R1 $R2" >> run_map.sh
echo -e "bowtie2 -p $processor --very-sensitive -X 1000  -x $ref_index -1 $R1_trim -2 $R2_trim -S ./01.Mapping/$out.sam --rg-id $out" >> run_map.sh
...
rm Rplots.pdf
rm *arrow
rm -r ArchRLogs tmp project 

############## motif analysis and spatial projection #########
for line in `cat dir.list`
do
cd $line
samtools merge $line.q10.rmdup.merge.bam ./01.Mapping/*.pe.q10.sort.rmdup.bam
samtools index $line.q10.rmdup.merge.bam
cd ..
/disk1/pengweixing/software/R-4.1.1/bin/Rscript /disk1/pengweixing/pipeline/spatial_atac/motif_chromvar.r ./Merge_bulk/Peak_calling/${line}_L001.filterBL.bed ./$line/$line.q10.rmdup.merge.bam $refrence_genome $line
done
if [ ! -d Motif_spatial ];then
mkdir Motif_spatial
fi
for line in `cat dir.list`
do
cd Motif_spatial
/disk1/pengweixing/software/anaconda3/bin/python3 /disk1/pengweixing/pipeline/spatial_atac/motif_plot.py -d ../${line}_deviation.txt -b ../barcode.txt
cd ..
done
