...
############################################
# 2017-05-08 by Yuning Wei
# edited:
# ATACseq_pipeline_v4.0.0.pl
# getQCTable.py

# added one output line in .log file for version information, thus added one to lines[] counts for .log file outpus
...
def getQCForSample(line, outFl):
    ...
    oneLine = lines[0].rstrip()
    words = oneLine.split()
    outFl.write(words[0] + "\t")
    totalReads = int(words[0])

    # Overall Alignment Rate
    oneLine = lines[14].rstrip()
    words = oneLine.split()
    outFl.write(words[0] + "\t")
    logFl.close()
    ...
    # QC filter
    oneLine = lines[28].rstrip()
    words = oneLine.split()
    beforeQC = int(words[0])

    oneLine = lines[22].rstrip()
    words = oneLine.split()
    afterQC = int(words[0])
